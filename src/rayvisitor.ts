...
export default class RayVisitor implements Visitor {
  ...
  visitGroupNode(node: GroupNode) {
    ...
    this.matrixStack.push(
      this.matrixStack[this.matrixStack.length - 1].mul(
        node.transform.getMatrix()
      )
    );
    this.inverseStack.push(
      node.transform
        .getInverseMatrix()
        .mul(this.inverseStack[this.inverseStack.length - 1])
    );
  ...
  visitSphereNode(node: SphereNode) {
    ...
    if (this.mouseRay) {
      ...
      if (intersection) {
        let objectIntersection: [Sphere, Intersection, Ray, SphereNode] = [
          sphere,
          intersection,
          mouseRayLocal,
          node,
